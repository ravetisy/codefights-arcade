"""Consider two following representations of a non-negative integer:
    ...
    from 0 to 9; An integer with at least one digit in a base from 2 to 16 (
    inclusive), enclosed between # characters, and preceded by the base,
    which can only be a number between 2 and 16 in the first representation.
    For digits from 10 to 15 characters a, b, ..., f and A, B, ...,
    F are used.

Additionally, both representations may contain underscore (_) characters;
they are used only as separators for improving legibility of numbers and can
be ignored while processing a number.
...
def solution(line: str = '') -> bool:
    ...
    for i in range(11, 17):
        if int(line[0]) == i:
            for j in range(0, len(line[1])):
                if not ('0' <= line[1][j] <= '9' or 'a' <= line[1][j] <= chr(
                        i - 11 + ord('a')) or 'A' <= line[1][j] <= chr(
                        i - 11 + ord('A'))):
                    return False
